...
class ChatController extends Controller
{
    public function index($caseId)
    {
        ...
        $chats = Chat::with('case')
            ...
            ->where(function($query) {
                $query->where('role', 'user') // semua pertanyaan user
                    ->orWhere('role', 'ai'); // ambil semua AI, nanti difilter di view
            })
            ->orderBy('created_at')
            ->get();
    ...
    public function store(Request $request, $caseId)
    {
        ...
        if ($exactMatch) {
            ...
            // ===== 2. MATCH gabungan (question + answer) =====
            $results = DB::select("
            ...
            if (!empty($results) && $results[0]->score > 0) {
                ...
                $confidence = min(1, $results[0]->score / 5); // sesuaikan skala max score
            }
        }
